...
self.balanceOf = function (paramObj) {
    Utils.assert(Utils.addressCheck(paramObj.owner), "ZTP721: Invalid owner address: " + paramObj.owner);
...
self.transferFrom = function (paramObj) {
    Utils.assert(Utils.addressCheck(paramObj.to), "ZTP721: Invalid receiver address.");
    let previousOwner = self.p.update(paramObj.to, paramObj.tokenId, Chain.msg.sender);
    Utils.assert(previousOwner === paramObj.from, "ZTP721: Incorrect owner");
};
```
...
#### Note: A wallet/broker/auction application MUST implement the **wallet interface** (ZTP721Receiver interface with the onZTP721Received function) if it will accept safe transfers.
