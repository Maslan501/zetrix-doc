...
Tokens standards like ZTP-20 and ZTP-721 require a separate contract to be deployed for each token type or collection. This places a lot of redundant bytecode on the Zetrix blockchain and limits certain functionality by the nature of separating each token contract into its own permissioned address. With the rise of blockchain games, game developers may be creating thousands of token types, and a new type of token standard is needed to support them. However, ZTP-1155 is not specific to games and many other applications can benefit from this flexibility.
...
self.safeTransferFrom = function (paramObj) {
    ...
    Utils.assert(paramObj.from === sender || self.isApprovedForAll({
        ...
        operator: sender
    }), "ZTP1155: Missing approval for all");
